...
class Sentiment_predict():
    ...
    def sentiment_df(self, source: str, model: str, tick = None, quer = None):
        if source == "News":
            ...
            if df_new.shape[0] > 0:
                ...
                df = polygon_news_df[['id', 'title', 'author', 'published_utc', 'article_url', 'tickers', 'amp_url', 'image_url', 'description', 'title_' + model, 'description_' + model]]
                return df
            else:
                df = polygon_prev[['id', 'title', 'author', 'published_utc', 'article_url', 'tickers', 'amp_url', 'image_url', 'description', 'title_' + model, 'description_' + model]]
                return df
        elif source == "Tweets":
    ...
    def sent_pie(self, source: str, model: str, ticker: str, part = None, query = None):
        import plotly.express as px
        if source == "News":
            df_polygon = self.sentiment_df(source, model)
            ...
            if query is not None:
                ...
                df_twt = self.sentiment_df(source, model, tick = ticker, quer = ticker + " AND crypto")
                colname = "tweet_" + model
                df_twt["total_news_day"] = df_twt.groupby('date')['id'].transform(len)
                twt_df = df_twt.groupby(['date', colname]).size().reset_index()
                twt_df.rename({0:'count'}, axis = 1, inplace = True)
    ...
    def sent_bar(self, source: str, model: str, ticker: str, part = None, query = None):
        import plotly.express as px
        if source == "News":
            df_polygon = self.sentiment_df(source, model)
            colname = part + "_" + model
